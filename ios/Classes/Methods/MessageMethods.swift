...
class MessageMethods: NSObject, TWCONMessageApi {
    ...
    public func getMediaContentTemporaryUrlConversationSid(_ conversationSid: String?, messageIndex: NSNumber?, completion: @escaping (String?, FlutterError?) -> Void) {
        debug("getMediaContentTemporaryUrl => conversationSid: \(String(describing: conversationSid)), messageIndex: \(String(describing: messageIndex))")
        ...
        guard let messageIndex = messageIndex else {
            return completion(nil, FlutterError(code: "MISSING_PARAMS", message: "Missing messageIndex", details: nil))
        }
                        ...
                        if result.isSuccessful, let message = message {
                            message.getMediaContentTemporaryUrl(completion: { (result: TCHResult, url: String?) in
                                if result.isSuccessful, let url = url {
                                    ...
                                    completion(nil, FlutterError(code: "ERROR", message: "MessageMethods.getMedia (Message.Media.getMediaContentTemporaryUrl)", details: nil))
                                }
                            })
                        } else {
                            self.debug("getMediaContentTemporaryUrl => onError: \(String(describing: result.error))")
                            completion(nil, FlutterError(code: "ERROR", message: "Error retrieving messages at index '\(messageIndex)' in conversation with sid '\(conversationSid)'", details: nil))
                        }
                    })
                } else {
